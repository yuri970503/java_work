...
public class MemberLocalVariable {
             int a; //맴버 변수 또는 필드 자동 초기화 값이 0 
             
             void printNumber (int c)//매개변수 값을 받는애 
             {
            	 int b = 1;//지역 변수 메서드가 없으면 죽는다. 소멸 값을 주는애가 없다. 사용자가 값을 주어야 사용가능.
            	 System.out.println("맴버변수 a: "+ a);
            	 System.out.println("맴버변수 b: "+ b);
            	 System.out.println("맴버변수 c: "+ c);
             }// 부르려면 실체가 있어야한다.
	public static void main(String[] args) {
		
		MemberLocalVariable m = new MemberLocalVariable();
		m.printNumber(4);//참조 연산(.)
